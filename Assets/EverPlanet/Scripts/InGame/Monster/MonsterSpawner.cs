...
public class MonsterSpawner : MonoBehaviour
{
    ...
    IEnumerator MonsterSpawn()
    {
        ...
        for(int spawnPositionNumber = 0; spawnPositionNumber < 38; spawnPositionNumber++)
        {
            ...
            for (int i = 0; i < spawnCount; i++)
            {
                ...
                //스폰 위치
                gm.transform.position = spawnPositionList[spawnPositionNumber].position + new Vector3(Random.Range(-5, 5), 0f, Random.Range(-5, 5));
                spawnMonster.Add(gm);
            }
        }
        yield break;
    ...
    IEnumerator FinalBossMonsterSpawn()
    {
        ...
        int monsterNumber = MonsterPosition(spawnPositionNumber, mapNum);//스폰할 몬스터
        GameObject gm = objectfulling.MakeObj(monsterNumber);
        gm.transform.position = spawnPositionList[spawnPositionNumber].position + new Vector3(Random.Range(-3, 3), 0.5f, Random.Range(-3, 3));
        spawnMonster.Add(gm);
        yield break;
    }
